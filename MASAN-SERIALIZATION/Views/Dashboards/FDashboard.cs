...
namespace MASAN_SERIALIZATION.Views.Dashboards
{
    public partial class FDashboard : UIPage
    {
            ...
            if (Globals_Database.Dictionary_ProductionCode_Data.TryGetValue(_data, out ProductionCodeData _produtionCodeData))
            {
                ...
                if (AppConfigs.Current.CameraSub_Timeout_Enabled && AppConfigs.Current.CameraSub_Timeout_Mode_2)
                {
                    ...
                    if (!previousPlcId.HasValue)
                    {
                        ...
                        this.InvokeIfRequired(() =>
                        {
                            ipConsole.Items.Add($"{DateTime.Now:HH:mm:ss}: CS V2 SYNC ERROR - Mã {_data}. Không đọc được ID hiện tại trước khi gửi phân làn.");
                            ipConsole.SelectedIndex = ipConsole.Items.Count - 1;
                        });
                ...
                if (successSend)
                {
                    ...
                    if (AppConfigs.Current.CameraSub_Timeout_Enabled)
                    {
                        if (AppConfigs.Current.CameraSub_Timeout_Mode_2)
                        {
                            ...
                            CameraSubSyncV2Result resultV2 = CheckCameraSubTimeoutV2(_data, previousPlcId.GetValueOrDefault());
        ...
        {
            int? previousId = ReadCameraSubCurrentIdV2();
            if (!previousId.HasValue)
            {
                return new CameraSubSyncV2Result
                {
                    Result = e_CameraSubSyncV2_Result.ReadError,
                    Message = "Không đọc được CurrentID trước khi chạy CameraSub timeout V2."
                };
            }

            return CheckCameraSubTimeoutV2(productCode, previousId.Value);
        }

        public CameraSubSyncV2Result CheckCameraSubTimeoutV2(string productCode, int previousId)
        {
            try
            {
        ...
        private int? ReadCameraSubCurrentIdV2()
        {
            string currentIdAddress = PLCAddress.Get("PLC_CurrentID_DM_C2");
            OperateResult<int[]> readCurrentId;

            if (AppConfigs.Current.PLC_Duo_Mode)
            {
                readCurrentId = OMRON_PLC_02.plc.ReadInt32(currentIdAddress, 1);
            }
            else
            {
                readCurrentId = OMRON_PLC.plc.ReadInt32(currentIdAddress, 1);
            }

            if (!readCurrentId.IsSuccess || readCurrentId.Content == null || readCurrentId.Content.Length == 0)
            {
                DashboardPageLog.WriteLogAsync(Globals.CurrentUser.Username, e_Dash_LogType.PlcError, "Lỗi đọc CurrentID CameraSub V2 trước khi gửi lane", readCurrentId.Message);
                return null;
            }

            return readCurrentId.Content[0];
        }
